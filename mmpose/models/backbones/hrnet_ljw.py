...
class HRNet_LJW(BaseBackbone):
    ...
    def __init__(
        ...
        output_channel_1 = (num_keypoints_1 + num_skeletons_1 * 2) * int(output_form_1)
        output_channel_2 = ((num_keypoints_1 + num_skeletons_1 * 2) * int(output_form_1) +
                            (num_keypoints_2 + num_skeletons_2 * 2) * int(output_form_2))
        # output_channel_3 = num_keypoints_1 + num_skeletons_1 * 2 + num_keypoints_2 + num_skeletons_2 * 2 + num_keypoints_3 + num_skeletons_3 * 2

        self.medium_stage_1_1 = nn.Conv2d(48, output_channel_1, 1, 1)
        self.medium_stage_1_2 = nn.Sequential(
            nn.BatchNorm2d(output_channel_1),
            nn.ReLU(inplace=True)
        )
        self.medium_stage_1_3 = nn.Sequential(
            nn.Conv2d(48 + output_channel_1, 48, 1, 1),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True)
        )

        self.medium_stage_2_1 = nn.Conv2d(48, output_channel_2, 1, 1)
        self.medium_stage_2_2 = nn.Sequential(
            nn.BatchNorm2d(output_channel_2),
            nn.ReLU(inplace=True)
        )
        self.medium_stage_2_3 = nn.Sequential(
            nn.Conv2d(48 + output_channel_2, 48, 1, 1),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True)
        )
        # stage 1
    ...
    def init_weights(self):
        ...
        super(HRNet_LJW, self).init_weights()
    ...
    def forward(self, x):
        ...
        if self.output_form_1 and self.use_medium_satge:
            out = self.medium_stage_1_1(y_list[0])
            save_for_loss.append(out)
            out = self.medium_stage_1_2(out)
            out = torch.cat([y_list[0], out], 1)
            y_list[0] = self.medium_stage_1_3(out)
        ...
        if (self.output_form_2 or self.output_form_1) and self.use_medium_satge:
            out = self.medium_stage_2_1(y_list[0])
            save_for_loss.append(out)
            out = self.medium_stage_2_2(out)
            out = torch.cat([y_list[0], out], 1)
            y_list[0] = self.medium_stage_2_3(out)
