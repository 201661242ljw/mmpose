    ...
    def forward(self,
        ...
        if mode == 'loss':
            ...
            for (idx, data_sample) in enumerate(data_samples):
                ...
                #     continue
                save_dir = r"E:\LJW\Git\mmpose\tools\0_LJW_tools\predict_show_1024/{}".format(img_name.split(".")[0])
                ...
                for i in range(90):
                    ...
                    if i % 15 < 5:
                        save_path = os.path.join(save_dir, "{}_ht_{}.jpg".format(i // 15, i % 15 + 1))
                    else:
                        save_path = os.path.join(save_dir, "{}_paf_{}_{}.jpg".format(i // 15,(i % 15 - 5) // 2 + 1, 'xy'[(i % 15 - 5) % 2] ))
                ...
                if True:
                    ...
                    for idx_ in range(3):
                        if idx_ != 2:
                            continue
                        output_ = output[15 * idx_ + 45:15 * idx_ + 60]
                        heatmap_avg = output_[:5]
                        heatmap_avg = np.transpose(heatmap_avg, (1, 2, 0))
                        paf_avg = output[5:]
                        paf_avg = np.transpose(paf_avg, (1, 2, 0))
                        heatmap_avg = cv2.resize(heatmap_avg, (img.shape[1], img.shape[0]),
                        ...
                        for part in range(heatmap_avg.shape[2]):
                            ...
                            one_heatmap = gaussian_filter(map_ori, sigma=[3,2,1.5][idx_])
                        ...
                        if True:
                            img_ = img // 4
                            for (idx,lst) in enumerate(all_peaks):
                                for [x,y,_,_] in lst:
                                    img_ = cv2.circle(img_,center=(x,y),color=[[0, 255, 0], [0, 0, 255], [255, 255, 0], [255, 0, 0], [255, 0, 255]][idx], radius=3, thickness=-1)
                            cv2.imwrite(os.path.join(save_dir, f"{idx_ + 3}_0_show_points.jpg"), img_)
                            exit()

                        # find connection in the specified sequence, center 29 is in the position 15
                        limbSeq = [[1, 0], [1, 1], [1, 2], [1, 3], [4, 0]]
                                                ...
                                                if len(special_connection) != 0:
                                                    for (idx, skeleton_temp) in enumerate(special_connection):
                                                        if skeleton_temp[1] == candA[i][2]:
                                                            if vec[0] * skeleton_temp[9] + vec[1] * skeleton_temp[
                                                                10] > 0.9:
                                                                need_record = False
                        ...
                        cv2.imwrite(os.path.join(save_dir, f"{idx_ + 3}_0_show.jpg"), img)
                    # exit()
