...
@TRANSFORMS.register_module()
class LJW_RandomFlip(BaseTransform):
    """Randomly flip the image, bbox and keypoints.
    ...
    def transform(self, results: dict) -> dict:
        ...
        if flip_dir is None:
            ...
            # flip keypoints
            if results.get('keypoints', None) is not None:
                keypoints, keypoints_visible = flip_keypoints(results['keypoints'],
                                                              results.get('keypoints_visible', None), image_size=(w, h),
                                                              flip_indices=results['flip_indices'], direction=flip_dir,
                                                              skip_swap=True)
                ...
                for (key_name, temp_dict) in results['raw_ann_info'].items():
                    if "true_points" in key_name:
                        ...
                        _ = np.ones([1, true_points.shape[1]], dtype=np.float32)
                        true_points, _ = flip_keypoints(true_points, _, image_size=(w, h),
                                                        flip_indices=results['flip_indices'],
                                                        direction=flip_dir, skip_swap=True)
                    ...
                    if "true_skeletons" in key_name:
                        ...
                        _ = np.ones([1, true_skeletons.shape[1]], dtype=np.float32)
                        true_skeletons[..., :2], _ = flip_keypoints(true_skeletons[..., :2], _, image_size=(w, h),
                                                                    flip_indices=results['flip_indices'],
                                                                    direction=flip_dir,
                                                                    skip_swap=True)
                        true_skeletons[..., 3:5], _ = flip_keypoints(true_skeletons[..., 3:5], _, image_size=(w, h),
                                                                     flip_indices=results['flip_indices'],
                                                                     direction=flip_dir,
                                                                     skip_swap=True)
...
@TRANSFORMS.register_module()
class LJW_RandomFlip_2(BaseTransform):
    """Randomly flip the image, bbox and keypoints.
...
@TRANSFORMS.register_module()
class LJW_RandomBBoxTransform(BaseTransform):
    r"""Rnadomly shift, resize and rotate the bounding boxes.
...
@TRANSFORMS.register_module()
class LJW_GenerateTarget(BaseTransform):
    """Encode keypoints into Target.
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if not isinstance(self.encoder, list):
            ...
            # 真实标注关键点变化
            true_points = results['raw_ann_info']['true_points']

            true_skeletons = results['raw_ann_info']['true_skeletons']
            ...
            encoded = self.encoder.encode(
                keypoints=true_points,
                skeletons=true_skeletons)
...
@TRANSFORMS.register_module()
class LJW_GenerateTarget_2(BaseTransform):
    """Encode keypoints into Target.
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if not isinstance(self.encoder, list):
            ...
            # true_points = results['raw_ann_info']['true_points']
            # true_skeletons = results['raw_ann_info']['true_skeletons']
            keypoints_1 = results['raw_ann_info']['true_points_1']
            skeletons_1 = results['raw_ann_info']['true_skeletons_1']
            keypoints_2 = results['raw_ann_info']['true_points_2']
            skeletons_2 = results['raw_ann_info']['true_skeletons_2']
            keypoints_3 = results['raw_ann_info']['true_points_3']
            skeletons_3 = results['raw_ann_info']['true_skeletons_3']
            auxiliary_encode_kwargs = {
                key: results[key]
                for key in self.encoder.auxiliary_encode_keys
            }
            ...
            encoded = self.encoder.encode(
                ...
                keypoints_3=keypoints_3,
                skeletons_3=skeletons_3, )
